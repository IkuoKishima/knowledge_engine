...
def iter_documents(input_root: Path) -> Iterable[Document]:
  for p in sorted(input_root.rglob("*.json")):
    ...
    for r in records:
      ...
      text = build_text(title, [str(x) for x in keywords], [str(x) for x in examples])
...
def upsert_doc(con: sqlite3.Connection, d: Document) -> None:
  ...
  """, (
    d.doc_id, d.source_path, d.source_record_id, d.genre_level1, d.genre_level2, d.title,
    d.version_hash, 0, d.created_at, now
  ))
